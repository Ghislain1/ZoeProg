namespace ZoeProg.Services
{
    ...
    public sealed class PlugInService : IPlugInService
    {
        ...
        public PlugInService(IUnityContainer container)
        {
            ...
            this.dict = new Dictionary<Guid, string>();
        }

        public List<FunctionItem> GetFunctionItemList()
        {
            List<FunctionItem> collec = new List<FunctionItem>();
            foreach (var item in this.dict.Where(i => i.Key != null))
            {
                var fItem = new FunctionItem() { Title = item.Value, Id = item.Key.ToString() };
                collec.Add(fItem);
            }
            return collec;
        }
        ...
        public Task<List<FunctionItem>> GetFunctionItemListAsynchronly()
        {
            ...
            {
                var res = this.GetFunctionItemList();

                System.Threading.Thread.Sleep(3000 * 4);
        ...
        public bool ImplementedIPlugIn(Type plugInType)
        {
            bool result = false;
            if (plugInType == null)
            {
        ...
        public void StoreTitle(string id, string title)
        {
            //ToDO
        }
